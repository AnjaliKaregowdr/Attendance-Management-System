...
<div class="container">
  <div class="row">
    <div class="col-md-12 col-lg-12">
			<h1 class="page-header">Take Half-day Attendance</h1>
		</div>
	</div>
	<div class="row">
		<div class="col-md-12 col-lg-12">
			<form action="" method="get" class="form-inline" id="subjectForm" data-toggle="validator">
				<div class="form-group">
					<label for="select" class="control-label">Subject:</label>
					<?php

						$query_subject = "SELECT subject.name, subject.id from subject
INNER JOIN user_subject WHERE user_subject.id = subject.id AND user_subject.uid = {$_SESSION['uid']}";
				...
				<input type="submit" class="btn btn-info" name="sbt_stn1" value="Load Student">
			</form>
			...
			<table class="table table-striped table-hover">
				<thead>
					<tr>
						<th>Roll No</th>
						<th>Name</th>
						<th><input type="checkbox" class="chk-head" /> halfday</th>
					</tr>
				</thead>
				...
				<?php
					 ...
					 $ddate = strtotime($dat);
					 $sub=$_GET['subject'];
					$que= "SELECT sid, aid, halfday  from attendance  WHERE date  =$ddate
					...
					for($i = 0; $i<count($rstu); $i++)
					{
						...
						if($updateFlag) {
							...
							echo"<td>".$rstu[$i]['name']."</td>";
								...
								if(($rstu[$i]['sid'] ==  $attData[$i]['sid']) && ($attData[$i]['halfday']))
								{
			...
			<?php
				...
				if (isset($_POST['sbt_top'])) {
					if(isset($_POST['updateData']) && ($_POST['updateData'] == 1) ) {
						...
						// prepare sql and bind parameters
							...
							{
									//echo "hii";
								// UPDATE `attendance` SET `halfday` = '1' WHERE `attendance`.`aid` = 79;

									$stmtInsert = $conn->prepare("UPDATE attendance SET halfday = :isMarked WHERE aid = :aid");

									if (count($halfday)) {
										$p = (in_array($st_sid[$j], $halfday)) ? 1 : 0;
									}
                ...
                <strong>Well done!</strong> Attendance Recorded Successfully!.
						...
						$tstamp = strtotime($date);
							...
							{
									//echo "hii";
									$stmtInsert = $conn->prepare("INSERT INTO attendance (sid, date, halfday, uid, id)
								VALUES (:sid, :date, :halfday, :uid, :id)");
									...
									$stmtInsert->bindParam(':halfday', $p);
