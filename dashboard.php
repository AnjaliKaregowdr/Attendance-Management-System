...
//select subject id and name of loggned in user
				$query_subjectPending = "SELECT subject.name, subject.id from subject INNER JOIN user_subject WHERE user_subject.id = subject.id AND user_subject.uid = {$_SESSION['uid']}";
...
//texual rep of a day
				...
				if(($weekday!="saturday") && ($weekday!="sunday")) {
					for($j = 0; $j<$noOfSubjectPending; $j++) {
						...
						if(!empty($resultP)){
            //    echo $dateCurrent;
							print "<p><a href='index.php?subject=" . $subIdP . "&date=" . $dateCurrentYMD ."'>Class <strong>" . $rsubPending[$j]['name'] ."</strong> of <strong>" . $dateCurrent ."</strong></a> <span class='label label-success'>Attendance Recorded</span> </p>";
						}
						else {
          //    echo $dateCurrent;
...
<div class="panel panel-success">
  ...
  <div class="panel-body">
    <p><i class="fa fa-book"></i> <a href="index.php?page=studentinfo"> <strong><?php print $noOfSubject; ?></strong> Subjects </a></p>
		<?php
			$studentQuery = "SELECT COUNT(DISTINCT rollno) as student_count FROM `user_subject` INNER JOIN student_subject WHERE user_subject.id = student_subject.id AND user_subject.uid = $userId";
